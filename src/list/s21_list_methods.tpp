...
void list<T>::unique() {
    ...
    while (1) {
        ...
        if (now->next_ ==  end_of_list_) break;
    }
}
                                               ...
                                               list<T>::const_reference value)
{
    ...
    if (pos.ptr_ == end_of_list_) {
        ...
        // If list no elements
        if (pos.ptr_->before_ == nullptr) head_of_list_ = new_node;
        else pos.ptr_->before_->next_ = new_node;
...
const T & list<T>::front(){
    return head_of_list_->data_;
}

template <class T>
const T &list<T>::back()
{ 
    return end_of_list_ == head_of_list_ ? head_of_list_->data_ : end_of_list_->before_->data_;
}
