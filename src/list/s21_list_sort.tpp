...
void list<T>::sort()
{
    ...
    while (1) {
        while (1) {
            ...
            if (current->next_ == end_of_list_) break;
        }
        if (flag == 0) break;
        flag = 0;
...
void list<T>::swap_elements (element_list *a, element_list *b) {
    ...
    if (a == b) throw std::out_of_range("В функцию swap_element поступило два одинаковых элемента\n");
    ...
    b->next_ = a;
    a->before_ = b;
    if (a->next_ != nullptr) a->next_->before_ = a;
}
